...
let isPortfolioRendered = false; // Flag to prevent duplicate renders

function renderPortfolio() {
    // Prevent duplicate rendering
    if (isPortfolioRendered) {
        console.warn('Portfolio already rendered, skipping duplicate render');
        return;
    }
    ...
    // Defer loading of other sections to prioritize Hero render
    setTimeout(() => {
        renderOtherSections(portfolioContainer);
    }, 0);
}

function renderOtherSections(portfolioContainer) {
    // --- About Me Section ---
